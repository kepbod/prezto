...
require() {
    ...
    which zsh || die "No zsh installed! Please install zsh from http://http://www.zsh.org/"
    color_print "Checking if git exists..."
    which git || die "No git installed! Please install git from http://git-scm.com/downloads/"
}
